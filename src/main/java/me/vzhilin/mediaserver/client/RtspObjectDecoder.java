...
// CS_OFF: UnusedImports
// CS_OFF: IllegalCatch
// CS_OFF: IllegalThrows
// CS_OFF: NoWhitespaceBefore
// CS_OFF: VisibilityModifier
// CS_OFF: MethodLength
// CS_OFF: MissingSwitchDefault
...
public abstract class RtspObjectDecoder extends ByteToMessageDecoder {
    ...
    private final InterleavedHeaderParser interleavedHeaderParser;
    ...
    private ByteBuf interleavedPayload;
     ...
     */
    private enum State {
        READ_INTERLEAVED_HEADER,
        READ_INTERLEAVED_DATA,
        SKIP_CONTROL_CHARS,
        READ_INITIAL,
        READ_HEADER,
    ...
    protected RtspObjectDecoder(
        ...
        interleavedHeaderParser = new InterleavedHeaderParser();
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        ...
        switch (currentState) {
        case READ_INTERLEAVED_HEADER: {
            interleavedHeader = interleavedHeaderParser.parse(buffer);
            if (interleavedHeader == null) {
                return;
            } else {
                currentState = State.READ_INTERLEAVED_DATA;
                interleavedPayload = ctx.alloc().directBuffer(interleavedHeader.getLength(), interleavedHeader.getLength());
            }
        }
    ...
    private void readInterleaved(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) {
        while (currentState == State.READ_INTERLEAVED_DATA) {
            ...
            interleavedPayload.writeBytes(buffer, (Math.min(buffer.readableBytes(), remaining)));
    ...
    protected boolean isInterleaved(ByteBuf buffer) {
        return buffer.readableBytes() > 0 && buffer.getByte(buffer.readerIndex()) == '$';
    }
     ...
     * @return true if the server switched to a different protocol
     */
